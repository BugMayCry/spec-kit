...
- ~~当仓库信息不完整或缺失时，系统应该如何处理？~~ → 已解决：展示可用信息并警告
- ~~对于小型项目或原型，仓库认知阶段是否可以简化？~~ → 已解决：用户可选模式，提供完整/快速模式
- ~~当架构师对项目已有深入了解时，是否提供跳过选项？~~ → 已解决：提供跳过按钮，仅限已确认了解项目的用户
- ~~提案状态如何标识？~~ → 已解决：文件名包含"proposal"且内容 Status 标注"Proposal"
...
- **FR-006**: Brainstorm命令生成的spec.md必须采用双重标识：文件名包含"proposal"字样（如`proposal-xxx-spec.md`），且文件内容Status字段标注"Proposal"，与specify命令生成的正式规范明确区分
- **FR-007**: 对于已确认了解项目的用户，系统应提供跳过仓库认知阶段的选项，跳过行为应被记录
- **FR-008**: 当仓库信息不完整或缺失时，系统应展示可用信息并对缺失部分发出警告提示
- **FR-009**: 系统应提供"完整模式"和"快速模式"供用户选择，快速模式仅展示核心信息
...
- Q: 仓库信息缺失时的处理 → A: 展示可用信息并警告 - 显示可获取的信息，对缺失部分发出警告提示
- Q: 提案状态标识方式 → A: 双重标识 - 文件名包含 "proposal" 且内容 Status 字段标注"Proposal"
- Q: 小型项目是否简化 → A: 用户可选模式 - 提供"完整模式"和"快速模式"供用户选择
